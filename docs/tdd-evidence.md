...
| Fail-closed release review | Malformed/oversized IPC, UTF-8 byte-cap, asynchronous config reload, and malformed input-policy probes reproduced blocking failures | Dedicated IPC policy, transactional compound validation, byte-accurate 4096-byte rejection, post-load preference normalization, hostile-state rejection, keyboard-accessible language/settings controls, and responsive rows pass |
| Exhaustive keyboard operation | New contracts failed before invariant close/restart, direct setup shortcuts, backward setting selection, owned language-grid activation, focus restoration, and narrow-choice visibility existed; Quattro then reproduced GridView consuming Enter/Space | Canonical Ctrl+R/Ctrl+Escape, direct setup keys, all 34 settings traversed and changed by keyboard, owned language navigation/activation, visible hints/focus, quick-end precedence, child-control modifier propagation, and 68/68 full-suite tests |
